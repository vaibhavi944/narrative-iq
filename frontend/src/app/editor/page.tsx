...
import { getMockAnalysis, type AnalysisResult } from "@/lib/mockData";
...
export default function EditorPage() {
  ...
  const handleAnalyze = () => {
    ...
    // Simulate API delay
    setTimeout(() => {
      // Pass real language to mock generator
      const mockRes = getMockAnalysis(text, language);
      setResult(mockRes);
  ...
  const applyRewrite = () => {
    if (result?.suggested_rewrite) {
      setText(result.suggested_rewrite);
    }
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-1.5 text-stone-500 text-[13px] font-medium hover:text-stone-900 transition-colors"
                >
                ...
                {isLangMenuOpen && (
                  <div className="absolute top-full left-0 mt-2 w-32 bg-white border border-stone-200 rounded-xl shadow-xl z-50 overflow-hidden">
                    {languages.map(l => (
                      <button
                        key={l.id}
                        onClick={() => {
                        ...
                        className={cn(
                          "w-full px-4 py-2 text-left text-[13px] hover:bg-stone-50 transition-colors",
                          language === l.id ? "text-indigo-700 font-bold bg-indigo-50/50" : "text-stone-600"
                        )}
                      >
                        {l.label}
                      </button>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-stone-50/30">
            <div className="max-w-3xl mx-auto w-full px-8 py-24 min-h-full flex flex-col">
              <textarea
                ...
                placeholder={language === 'marathi' ? 'येथे लिहायला सुरुवात करा...' : language === 'hindi' ? 'यहाँ लिखना शुरू करें...' : "Start your narrative here..."}
              ...
              >
                ...
                <div className="p-8 border-b border-stone-100 bg-stone-50/30">
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-stone-400 block mb-2">Editor's Assessment</span>
                      <h3 className={cn(
                        "text-2xl font-bold tracking-tight",
                        result.label === "Strong" ? "text-emerald-600" : "text-amber-600"
                      )}>
                        {result.label === "Strong" ? "Beautifully Written" : result.label === "Moderate" ? "Solid Foundation" : "Needs More Depth"}
                      </h3>
                    </div>
                  </div>

                  <div className="space-y-5">
                    <QualityIndicator label="Story Flow" score={result.pacing.pacing_score} />
                    <QualityIndicator label="Word Variety" score={result.repetition.repetition_score} />
                    <QualityIndicator label="Emotional Heart" score={result.emotion.emotion_score} />
                  </div>
                </div>
                ...
                <div className="p-8 space-y-12">
                  ...
                  <section>
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-500">Suggested Rewrite</h3>
                      <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Story Preserved</span>
                    </div>
                    <div className="bg-gradient-to-br from-indigo-50/80 to-white p-7 rounded-[2rem] border border-indigo-100 shadow-sm relative overflow-hidden group">
                      <div className="absolute top-0 right-0 p-4 opacity-20 group-hover:rotate-12 transition-transform">
                        <Sparkles className="w-8 h-8 text-indigo-600" />
                      </div>
                      <p className="text-[18px] font-serif text-stone-900 leading-[1.85] italic relative z-10 whitespace-pre-wrap">
                        {result.suggested_rewrite}
                      </p>
                      <button 
                        onClick={applyRewrite}
                        className="mt-8 px-5 py-2.5 bg-indigo-600 text-white rounded-full text-[12px] font-bold hover:bg-indigo-700 transition-all flex items-center gap-2 shadow-lg shadow-indigo-100 group/btn"
                      >
                        Use this version
                  ...
                  {/* AI Critique */}
                  <section>
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-stone-400 mb-6">How To Strengthen The Scene</h3>
                  ...
                  <section className="pt-4">
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-stone-400 mb-6">A Stronger Example</h3>
  ...
  return (
    <div className="flex items-center justify-between group">
      <div className="space-y-1">
        <span className="text-[11px] font-bold uppercase tracking-wider text-stone-400 block">{label}</span>
        <span className={cn("text-[13px] font-bold tracking-tight", colorClass)}>{status}</span>
      </div>
